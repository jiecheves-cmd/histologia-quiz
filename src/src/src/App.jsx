...
export default function App() {
  ...
  if (!loaded) return <div style={{padding:"2rem",textAlign:"center",color:"#888"}}>Cargando...</div>;
  ...
  return (
    <div style={{fontFamily:"system-ui,sans-serif",maxWidth:800,margin:"0 auto",padding:"1rem"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1.25rem",flexWrap:"wrap",gap:8}}>
        <div>
          <h2 style={{fontSize:19,fontWeight:600,margin:"0 0 2px"}}>Quiz de Histología Médica</h2>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontSize:13,color:"#666"}}>{currentUser.displayName}</span>
            <Badge role={role} />
          </div>
        </div>
        <button onClick={() => setCurrentUser(null)}
          style={{fontSize:12,padding:"5px 14px",borderRadius:6,cursor:"pointer",background:"transparent",color:"#666",border:"1px solid #ddd"}}>
          Cerrar sesión
        </button>
      </div>
        ...
        <div style={{display:"flex",gap:8,marginBottom:"1.5rem",flexWrap:"wrap"}}>
          {[["Modo Alumno","alumno"],["Modo Profesor","profesor"],...(canSuper?[["Supervisor","supervisor"]]:[])].map(([l,v]) => (
            <button key={v} onClick={() => setTab(v)}
              style={{padding:"6px 16px",borderRadius:6,fontSize:13,fontWeight:500,cursor:"pointer",
                background:tab===v?"#EBF4FF":"transparent",
                color:tab===v?"#1a56db":"#666",
                border:tab===v?"1px solid #93c5fd":"1px solid #ddd"}}>
              {l}
            </button>
          ))}
        </div>
...
function LoginScreen({ users, onLogin }) {
  ...
  return (
    <div style={{fontFamily:"system-ui,sans-serif",maxWidth:380,margin:"3rem auto",padding:"0 1rem"}}>
      <h2 style={{fontSize:20,fontWeight:600,marginBottom:4}}>Quiz de Histología</h2>
      <p style={{fontSize:13,color:"#666",marginBottom:"1.75rem"}}>Introduce tus credenciales para acceder</p>
      <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:16}}>
        <input value={username} onChange={e => setUsername(e.target.value)} onKeyDown={e => e.key==="Enter"&&login()}
          placeholder="Usuario"
          style={{fontSize:14,padding:"10px 14px",borderRadius:6,border:"1px solid #ddd"}} />
      ...
      <button onClick={login}
        style={{width:"100%",padding:"10px",borderRadius:6,fontSize:14,fontWeight:600,cursor:"pointer",
...
function StudentMode({ db, studentName }) {
  ...
  if (phase==="config") return (
    <div>
      <p style={{fontSize:14,color:"#666",marginBottom:"1.25rem"}}>
        Hola, <strong>{studentName}</strong>. Configura tu sesión:
      </p>
      <div style={{marginBottom:"1.25rem"}}>
        <p style={{fontSize:13,fontWeight:500,margin:"0 0 8px"}}>Dificultad</p>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          {["todas","básico","intermedio","avanzado"].map(d => (
            <button key={d} onClick={() => setFilter(d)}
              style={{padding:"6px 16px",borderRadius:6,fontSize:13,cursor:"pointer",
                background:filter===d?"#f0f0f0":"transparent",fontWeight:filter===d?600:400,
      ...
      <div style={{marginBottom:"1.25rem"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <p style={{fontSize:13,fontWeight:500,margin:0}}>Temas</p>
          <div style={{display:"flex",gap:8}}>
            <button onClick={() => setSelectedTopics([...availableTopics])}
              style={{fontSize:11,padding:"2px 10px",borderRadius:10,cursor:"pointer",background:"#f0f0f0",border:"1px solid #ddd"}}>Todos</button>
            <button onClick={() => setSelectedTopics([])}
              style={{fontSize:11,padding:"2px 10px",borderRadius:10,cursor:"pointer",background:"#f0f0f0",border:"1px solid #ddd"}}>Ninguno</button>
          </div>
        </div>
      ...
      <div style={{marginBottom:"1.75rem"}}>
        <p style={{fontSize:13,fontWeight:500,margin:"0 0 8px"}}>Número de preguntas</p>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          {[5,10,15,20].map(n => {
            const disabled = poolSize < n;
            return (
              <button key={n} onClick={() => !disabled && setNumQ(n)}
                style={{width:52,height:40,borderRadius:6,fontSize:15,
                  fontWeight:numQ===n?700:400, cursor:disabled?"not-allowed":"pointer", opacity:disabled?0.35:1,
                  background:numQ===n?"#EBF4FF":"transparent",
                  color:numQ===n?"#1a56db":"#333",
                  border:numQ===n?"1px solid #93c5fd":"1px solid #ddd"}}>
                {n}
              </button>
            );
          })}
        </div>
        <p style={{fontSize:12,color:"#666",margin:"6px 0 0"}}>{poolSize} preguntas disponibles</p>
      </div>
      <button onClick={start} disabled={!poolSize}
        style={{padding:"9px 28px",borderRadius:6,fontSize:14,fontWeight:600,
  ...
  if (phase==="results") {
    ...
    return (
      <ResultsWithRanking
        studentName={studentName} answers={answers} questions={questions}
        sessionStart={sessionStart} correct={correct} pct={pct}
        points={points} bonuses={bonuses} ptAciertos={ptAciertos} ptErrores={ptErrores}
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"0.75rem"}}>
        <span style={{fontSize:13,color:"#666"}}>Pregunta {current+1} de {questions.length}</span>
      ...
      <p style={{fontSize:15,fontWeight:500,marginBottom:"1.25rem",lineHeight:1.5}}>{q.question}</p>
      <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:"1.25rem"}}>
        {q.options.map((opt, i) => {
          let bg="#fff", border="1px solid #ddd", tc="#333", cls="opt-btn";
          if (confirmed) {
            cls += " opt-confirmed";
            if (i===q.answer) { bg="#E1F5EE"; border="1px solid #1D9E75"; tc="#085041"; cls+=" opt-correct"; }
            else if (i===selected) { bg="#FAECE7"; border="1px solid #D85A30"; tc="#4A1B0C"; cls+=" opt-wrong"; }
            else { tc="#999"; }
          } else if (i===selected) { bg="#EBF4FF"; border="1px solid #93c5fd"; tc="#1a56db"; cls+=" opt-selected"; }
          return (
            <div key={i} onClick={() => !confirmed && setSelected(i)} className={cls}
              style={{padding:"11px 14px",borderRadius:6,border,background:bg,
              ...
              <span style={{minWidth:24,height:24,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",
                ...
                background: confirmed&&i===q.answer?"#1D9E75": confirmed&&i===selected?"#D85A30": i===selected?"#1a56db":"#f0f0f0",
                color: (confirmed&&(i===q.answer||i===selected))||(!confirmed&&i===selected)?"#fff":"#666"}}>
                {confirmed&&i===q.answer?"✓": confirmed&&i===selected?"✗": ["A","B","C","D"][i]}
              </span>
              <span>{opt}</span>
            </div>
          );
      ...
      {selected!==null && !confirmed && (
        <div style={{marginBottom:"1.25rem",padding:"12px 14px",borderRadius:6,background:"#f9f9f9",border:"1px solid #ddd"}}>
          <p style={{fontSize:13,fontWeight:500,margin:"0 0 10px"}}>¿Con qué seguridad respondes?</p>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {CONFIDENCE.map((c, i) => (
              <button key={i} onClick={() => setConfidence(i)} className="conf-btn"
                style={{flex:1,minWidth:80,padding:"10px 8px",borderRadius:6,cursor:"pointer",
                  background:confidence===i?c.bg:"#fff",
                  border:"1.5px solid "+(confidence===i?c.border:"#ddd"),
                  display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                <span style={{fontSize:20}}>{c.icon}</span>
                <span style={{fontSize:13,fontWeight:confidence===i?700:400,color:confidence===i?c.color:"#666"}}>{c.label}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {confirmed && (
        <div className="explain-box"
          style={{borderLeft:"3px solid",borderColor:selected===q.answer?"#1D9E75":"#D85A30",
            borderRadius:"0 6px 6px 0",background:"#f9f9f9",padding:"12px 16px",marginBottom:"1.25rem",fontSize:13,lineHeight:1.7}}>
          ...
          {q.explanationImage && <div style={{marginTop:10}}><ImageViewer src={q.explanationImage} label="Ver imagen de explicación" /></div>}
        </div>
      )}
      <div style={{display:"flex",gap:8}}>
        {!confirmed
          ? <button onClick={confirm} disabled={selected===null||confidence===null}
              style={{padding:"8px 22px",borderRadius:6,fontSize:13,fontWeight:500,
          ...
          : <button onClick={next} className="next-btn"
              style={{padding:"8px 22px",borderRadius:6,fontSize:13,fontWeight:500,cursor:"pointer",
                background:selected===q.answer?"#E1F5EE":"#FAECE7",
                color:selected===q.answer?"#085041":"#4A1B0C",
                border:"1px solid "+(selected===q.answer?"#1D9E75":"#D85A30")}}>
...
function ResultsWithRanking({ studentName, answers, questions, sessionStart, correct, pct, points, bonuses, ptAciertos, ptErrores, onNewSession }) {
  ...
  return (
    <div>
      ...
      <p style={{fontSize:13,color:"#666",marginBottom:"1rem"}}>{studentName} · {fmt(Date.now()-(sessionStart||Date.now()))}</p>
      <div style={{display:"flex",gap:12,marginBottom:"1rem",flexWrap:"wrap"}}>
        {[["Correctas",correct,"#1D9E75"],["Incorrectas",answers.length-correct,"#C0392B"],["Acierto",pct+"%",pct>=70?"#1D9E75":"#C0392B"]].map(([l,v,c]) => (
          <div key={l} style={{flex:1,minWidth:100,background:"#f9f9f9",borderRadius:6,padding:"12px 16px",border:"1px solid #eee"}}>
            <div style={{fontSize:12,color:"#666",marginBottom:4}}>{l}</div>
            <div style={{fontSize:22,fontWeight:600,color:c}}>{v}</div>
          </div>
        ))}
      </div>
      <div style={{background:"#F0EAF9",border:"1px solid #C9A8F0",borderRadius:6,padding:"12px 16px",marginBottom:"1rem"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
      ...
      <div style={{background:"#f9f9f9",borderRadius:6,padding:"14px 16px",marginBottom:"1.5rem",border:"1px solid #eee"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}>
          <span style={{fontSize:13,fontWeight:600}}>Tu posición en el ranking</span>
            ...
            {["week","month","year"].map(p=>(
              <button key={p} onClick={()=>setRankPeriod(p)}
                style={{padding:"3px 10px",borderRadius:20,fontSize:11,cursor:"pointer",
                  ...
                  color:rankPeriod===p?"#7B4FBE":"#666",
                  border:rankPeriod===p?"1px solid #C9A8F0":"1px solid #ddd"}}>
                {periodLabel[p]}
              </button>
            ))}
          </div>
        ...
        {myPos>=0 ? (
          <div style={{display:"flex",alignItems:"center",gap:12,padding:"14px 16px",borderRadius:6,marginBottom:12,
            background:myPos===0?"#F0EAF9":myPos===1?"#F7F4FD":myPos===2?"#FAF8FF":"#EBF4FF",
            border:"1.5px solid "+(myPos<3?"#C9A8F0":"#93c5fd")}}>
            <span style={{fontSize:myPos<3?28:20,minWidth:36,textAlign:"center"}}>
              {myPos<3 ? medals[myPos] : "#"+(myPos+1)}
            </span>
            <div style={{flex:1}}>
              <div style={{fontSize:14,fontWeight:700}}>{studentName}</div>
              <div style={{fontSize:12,color:"#666"}}>
                {myPos===0?"¡Eres el primero!":myPos===1?"¡Segundo puesto!":myPos===2?"¡Tercer puesto!":"Posición "+(myPos+1)+" de "+ranking.length}
              </div>
            </div>
          ...
          <div style={{padding:"10px 14px",borderRadius:6,background:"#fff",fontSize:13,color:"#666",marginBottom:12,textAlign:"center",border:"1px solid #eee"}}>
            Aún no apareces en el ranking de este período.
          </div>
        )}
        {ranking.length>0 && (
          <div style={{display:"flex",flexDirection:"column",gap:5}}>
            {ranking.slice(0,5).map((r,i)=>{
              ...
              return (
                <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 10px",borderRadius:6,
                  background:isMe?"#F0EAF9":"#fff",border:"1px solid "+(isMe?"#C9A8F0":"#eee"),fontWeight:isMe?600:400}}>
                  <span style={{fontSize:i<3?16:12,minWidth:28,textAlign:"center"}}>{i<3?medals[i]:(i+1)+"."}</span>
                  <span style={{flex:1,fontSize:13,color:isMe?"#5B2D9E":"#333"}}>{r.name}{isMe?" (tú)":""}</span>
            ...
            {myPos>=5 && (
              <>
                {myPos>5 && <div style={{textAlign:"center",fontSize:11,color:"#666",padding:"2px 0"}}>···</div>}
                <div style={{display:"flex",alignItems:"center",gap:10,padding:"7px 10px",borderRadius:6,background:"#F0EAF9",border:"1px solid #C9A8F0",fontWeight:600}}>
      ...
      <button onClick={onNewSession}
        style={{padding:"7px 20px",borderRadius:6,fontSize:13,cursor:"pointer",background:"transparent",border:"1px solid #ddd"}}>
        Nueva sesión
      </button>
    </div>
  );
  ...
  return (
    <>
      <div onClick={() => setOpen(true)}
        style={{borderRadius:6,overflow:"hidden",border:"1px solid #ddd",cursor:"zoom-in",position:"relative",
...
function ReviewCard({ q, userAnswer, correct, confidence, idx }) {
  ...
  return (
    <div style={{borderRadius:6,border:"1px solid #ddd",marginBottom:8,overflow:"hidden"}}>
      <div onClick={() => setOpen(o => !o)}
        ...
        <span style={{flex:1,fontSize:13,lineHeight:1.4}}>{idx+1}. {q.question}</span>
        {confidence!=null && (
        ...
        <span style={{fontSize:12,color:"#666",flexShrink:0}}>{open?"▲":"▼"}</span>
      </div>
      {open && (
        <div style={{padding:"0 14px 14px",background:"#f9f9f9"}}>
          {q.image && <img src={q.image} alt="" style={{width:"100%",maxHeight:180,objectFit:"cover",borderRadius:6,margin:"10px 0"}} />}
          <div style={{display:"flex",flexDirection:"column",gap:6,margin:"10px 0"}}>
            {q.options.map((opt, i) => {
              const isC=i===q.answer, isU=i===userAnswer;
              let bg="#fff", border="1px solid #ddd", tc="#666";
              if (isC) { bg="#E1F5EE"; border="1px solid #1D9E75"; tc="#085041"; }
              else if (isU&&!isC) { bg="#FAECE7"; border="1px solid #D85A30"; tc="#4A1B0C"; }
              return (
                <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",borderRadius:6,border,background:bg}}>
                  <span style={{minWidth:20,height:20,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",
                    fontSize:11,fontWeight:500,flexShrink:0,
                    background:isC?"#1D9E75":isU?"#D85A30":"#f0f0f0",
                    color:isC||isU?"#fff":"#666"}}>
                    {isC?"✓":isU?"✗":labels[i]}
                  </span>
          ...
          <div style={{borderLeft:"3px solid",borderColor:correct?"#1D9E75":"#D85A30",
            borderRadius:"0 6px 6px 0",background:"#fff",padding:"10px 12px",fontSize:13,color:"#555",lineHeight:1.6}}>
            {q.explanation}
            {q.explanationImage && <div style={{marginTop:8}}><ImageViewer src={q.explanationImage} label="Ver imagen de explicación" /></div>}
          </div>
        </div>
      )}
    </div>
...
function TeacherMode({ db, updateDb, isSupervisor }) {
  ...
  const [rankPeriod, setRankPeriod]             = useState("week");
  const [form, setForm] = useState({difficulty:"básico",topic:TOPICS[0],question:"",options:["","","",""],answer:0,explanation:"",explanationImage:null,image:null});
  ...
  const handleImg     = e => { const f=e.target.files[0]; if(!f) return; const r=new FileReader(); r.onload=ev=>setForm(f=>({...f,image:ev.target.result})); r.readAsDataURL(f); };
  const handleExplImg = e => { const f=e.target.files[0]; if(!f) return; const r=new FileReader(); r.onload=ev=>setForm(f=>({...f,explanationImage:ev.target.result})); r.readAsDataURL(f); };
  ...
  const generateAI = async () => {
    ...
    try {
      ...
      const newQs  = parsed.map((q,i) => ({...q,difficulty:genDiff,topic:genTopic,id:Date.now()+i,image:null,explanationImage:null,supervised:false}));
      updateDb([...db,...newQs]);
      ...
      {[["Estadísticas","stats"],["Ranking","ranking"],["Preguntas","list"],["Generar IA","generate"],["Importar","import"]].map(([l,v]) => (
        <button key={v} onClick={() => setView(v)}
          style={{padding:"6px 14px",borderRadius:6,fontSize:13,cursor:"pointer",
            background:view===v?"#EBF4FF":"transparent",
            color:view===v?"#1a56db":"#666",
            border:view===v?"1px solid #93c5fd":"1px solid #ddd"}}>
          {l}
        </button>
      ))}
    </div>
  ...
  if (view==="import") return <ImportView db={db} updateDb={updateDb} />;

  if (view==="ranking") {
    const now    = new Date().toISOString();
    const curKey = periodKey(now, rankPeriod);
    const stuPoints = {};
    sessions.filter(s => periodKey(s.date, rankPeriod)===curKey).forEach(s => {
    ...
    return (
      <div>
        ...
        {ranking.length===0
          ? <div style={{padding:"2rem",textAlign:"center",color:"#666",background:"#f9f9f9",borderRadius:6}}>Sin datos para este período.</div>
          : <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {ranking.map((s,i)=>{
                const pct=s.total?Math.round(s.correct/s.total*100):0;
                return (
                  <div key={s.name} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",borderRadius:6,
                    border:"1px solid "+(i<3?"#C9A8F0":"#ddd"),
                    background:i===0?"#F0EAF9":i===1?"#F7F4FD":i===2?"#FAF8FF":"#fff"}}>
        ...
        <div style={{marginTop:"1.25rem",padding:"10px 14px",borderRadius:6,background:"#f9f9f9",fontSize:11,color:"#666",lineHeight:1.8,border:"1px solid #eee"}}>
  ...
  if (view==="stats") {
    ...
    allA.forEach(a=>{
      ...
      qStats[a.questionId].total++;if(a.correct)qStats[a.questionId].correct++;
    ...
    sessions.forEach(s=>{
      ...
      stuStats[s.student].sessions++;stuStats[s.student].totalTime+=s.durationMs||0;
    ...
    return (
      <div>
        <Nav/>
        <div style={{display:"flex",gap:8,marginBottom:"1rem",padding:"10px 14px",borderRadius:6,background:"#F0EAF9",border:"1px solid #C9A8F0",fontSize:13,alignItems:"center"}}>
        ...
        {sessions.length===0
          ? <div style={{padding:"2rem",textAlign:"center",color:"#666",background:"#f9f9f9",borderRadius:6}}>Sin datos de sesiones aún.</div>
          : (<>
            <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:"1.5rem"}}>
              {[[sessions.length+"","Sesiones","#378ADD"],[students.length+"","Alumnos","#1D9E75"],[fmt(totalTime),"Tiempo total","#BA7517"],[Math.round(allA.filter(a=>a.correct).length/Math.max(allA.length,1)*100)+"%","Acierto global","#7B4FBE"]].map(([v,l,c])=>(
                <div key={l} style={{flex:1,minWidth:100,background:"#f9f9f9",borderRadius:6,padding:"12px 14px",border:"1px solid #eee"}}>
                  <div style={{fontSize:11,color:"#666",marginBottom:4}}>{l}</div>
                  <div style={{fontSize:22,fontWeight:700,color:c}}>{v}</div>
                </div>
              ))}
            </div>
            <div style={{background:"#f9f9f9",borderRadius:6,padding:"14px 16px",marginBottom:"1.25rem",border:"1px solid #eee"}}>
              <p style={{fontSize:13,fontWeight:600,margin:"0 0 12px"}}>Seguridad vs Acierto</p>
              <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                {[0,1,2].map(i=>(
                  <div key={i} style={{flex:1,minWidth:90,background:"#fff",borderRadius:6,padding:"10px 12px",borderLeft:"3px solid "+CONFIDENCE[i].color}}>
                    <div style={{fontSize:12,fontWeight:600,color:CONFIDENCE[i].color,marginBottom:6}}>{CONFIDENCE[i].icon} {CONFIDENCE[i].label}</div>
                    <div style={{fontSize:11,color:"#666"}}>Resp: <strong>{confTotals[i]}</strong></div>
                    <div style={{fontSize:11,color:"#666"}}>Acierto: <strong style={{color:CONFIDENCE[i].color}}>{confTotals[i]?Math.round(confCorrect[i]/confTotals[i]*100):0}%</strong></div>
                  </div>
                ))}
              </div>
            </div>
            <div style={{background:"#f9f9f9",borderRadius:6,padding:"14px 16px",marginBottom:"1.25rem",border:"1px solid #eee"}}>
              ...
              {qList.slice(0,5).map((q,i)=>{
                const pct=Math.round(q.correct/q.total*100);const ds=diffStyle(q.difficulty);
                return (
                  <div key={i} style={{marginBottom:10,paddingBottom:10,borderBottom:i<4?"1px solid #eee":"none"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      ...
                      <span style={{fontSize:11,color:"#666"}}>{q.total} resp.</span>
                    </div>
                  </div>
                );
              })}
            ...
            <div style={{background:"#f9f9f9",borderRadius:6,padding:"14px 16px",marginBottom:"1.25rem",border:"1px solid #eee"}}>
              ...
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                <thead><tr style={{color:"#666",fontSize:11}}>{["Alumno","Sesiones","Tiempo","Acierto"].map(h=><th key={h} style={{textAlign:"left",padding:"4px 8px",fontWeight:500}}>{h}</th>)}</tr></thead>
                <tbody>{stuList.map((s,i)=>{const pct=s.total?Math.round(s.correct/s.total*100):0;return(<tr key={i} style={{borderTop:"1px solid #eee"}}><td style={{padding:"8px",fontWeight:500}}>{s.name}</td><td style={{padding:"8px",color:"#666"}}>{s.sessions}</td><td style={{padding:"8px",color:"#666"}}>{fmt(s.totalTime)}</td><td style={{padding:"8px",fontWeight:600,color:pct>=70?"#1D9E75":pct>=40?"#BA7517":"#C0392B"}}>{pct}%</td></tr>);})}</tbody>
              </table>
            </div>
            <div style={{background:"#f9f9f9",borderRadius:6,padding:"14px 16px",border:"1px solid #eee"}}>
              <p style={{fontSize:13,fontWeight:600,margin:"0 0 12px"}}>Historial de sesiones</p>
              {[...sessions].reverse().slice(0,10).map((s,i)=>{const ans=s.answers||[];const ok=ans.filter(a=>a.correct).length;const pct=ans.length?Math.round(ok/ans.length*100):0;return(<div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",borderRadius:6,background:"#fff",fontSize:13,marginBottom:6,border:"1px solid #eee"}}><span style={{flex:1,fontWeight:500}}>{s.student}</span><span style={{color:"#666",fontSize:12}}>{new Date(s.date).toLocaleDateString("es-ES",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"})}</span><span style={{color:"#666",fontSize:12}}>{fmt(s.durationMs||0)}</span><span style={{fontSize:12,fontWeight:600,padding:"2px 10px",borderRadius:10,background:pct>=70?"#E1F5EE":pct>=40?"#FEF3DC":"#FAECE7",color:pct>=70?"#0F6E56":pct>=40?"#7A4A00":"#993C1D"}}>{pct}% · {ans.length}p</span></div>);})}
            </div>
          </>)
        }
      </div>
  ...
  if (view==="generate") return (
    <div>
      <Nav/>
      <div style={{background:"#f9f9f9",borderRadius:6,padding:"1rem 1.25rem",border:"1px solid #eee"}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:8,alignItems:"center",marginBottom:10}}>
          <input value={genTopic} onChange={e=>setGenTopic(e.target.value)} placeholder="Tema (ej: epitelio intestinal...)"
            style={{flex:2,minWidth:160,fontSize:13,padding:"6px 10px",borderRadius:6,border:"1px solid #ddd"}}/>
          <select value={genDiff} onChange={e=>setGenDiff(e.target.value)} style={{fontSize:13,padding:"6px 8px",borderRadius:6,border:"1px solid #ddd"}}>
            {DIFFS.map(d=><option key={d} value={d}>{d}</option>)}
          </select>
          <select value={genCount} onChange={e=>setGenCount(Number(e.target.value))} style={{fontSize:13,padding:"6px 8px",borderRadius:6,border:"1px solid #ddd"}}>
            {[1,2,3,5].map(n=><option key={n} value={n}>{n} pregunta{n>1?"s":""}</option>)}
          </select>
          <button onClick={generateAI} disabled={generating}
            style={{padding:"6px 16px",borderRadius:6,fontSize:13,fontWeight:500,cursor:generating?"not-allowed":"pointer",
              opacity:generating?0.6:1,background:"#1a56db",color:"#fff",border:"none"}}>
            {generating?"Generando...":"Generar"}
          </button>
        </div>
        {genMsg&&<p style={{fontSize:12,color:genMsg.startsWith("Se añadieron")?"#0F6E56":"#C0392B",margin:0}}>{genMsg}</p>}
      </div>
    </div>
  );
  ...
  if (view==="edit") return (
    <div>
      <button onClick={()=>setView("list")} style={{fontSize:13,color:"#666",background:"none",border:"none",cursor:"pointer",padding:0,marginBottom:"1rem"}}>← Volver</button>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <span style={{fontSize:13,color:"#666",minWidth:80}}>Dificultad</span>
            <select value={form.difficulty} onChange={e=>setForm(f=>({...f,difficulty:e.target.value}))} style={{fontSize:13,padding:"4px 8px",borderRadius:6,border:"1px solid #ddd"}}>
              {DIFFS.map(d=><option key={d} value={d}>{d}</option>)}
            </select>
          </div>
          <div style={{display:"flex",gap:8,alignItems:"center",flex:1}}>
            <span style={{fontSize:13,color:"#666",minWidth:40}}>Tema</span>
            <select value={form.topic||TOPICS[0]} onChange={e=>setForm(f=>({...f,topic:e.target.value}))} style={{flex:1,fontSize:13,padding:"4px 8px",borderRadius:6,border:"1px solid #ddd"}}>
              {TOPICS.map(t=><option key={t} value={t}>{t}</option>)}
            </select>
          </div>
        </div>
        <div>
          <div style={{fontSize:13,color:"#666",marginBottom:4}}>Pregunta</div>
          <textarea value={form.question} onChange={e=>setForm(f=>({...f,question:e.target.value}))} rows={3}
            style={{width:"100%",fontSize:13,padding:"8px 10px",borderRadius:6,border:"1px solid #ddd",resize:"vertical",boxSizing:"border-box"}}/>
        </div>
        <div>
          <div style={{fontSize:13,color:"#666",marginBottom:4}}>Opciones (marca la correcta)</div>
          {form.options.map((opt,i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
              ...
              <input value={opt} onChange={e=>{const opts=[...form.options];opts[i]=e.target.value;setForm(f=>({...f,options:opts}));}}
                placeholder={"Opción "+(i+1)} style={{flex:1,fontSize:13,padding:"6px 10px",borderRadius:6,border:"1px solid #ddd"}}/>
            </div>
          ))}
        </div>
        <div>
          <div style={{fontSize:13,color:"#666",marginBottom:4}}>Explicación</div>
          <textarea value={form.explanation} onChange={e=>setForm(f=>({...f,explanation:e.target.value}))} rows={3}
            style={{width:"100%",fontSize:13,padding:"8px 10px",borderRadius:6,border:"1px solid #ddd",resize:"vertical",boxSizing:"border-box"}}/>
          <div style={{fontSize:12,color:"#666",margin:"8px 0 4px"}}>Imagen de la explicación (opcional)</div>
          {form.explanationImage&&<img src={form.explanationImage} alt="" style={{width:"100%",maxHeight:180,objectFit:"contain",borderRadius:6,marginBottom:6,border:"1px solid #ddd",background:"#f8f8f8"}}/>}
          <div style={{display:"flex",gap:8}}>
            <button onClick={()=>explImgRef.current.click()} style={{fontSize:12,padding:"4px 12px",borderRadius:6,cursor:"pointer",background:"transparent",border:"1px solid #ddd"}}>Subir imagen</button>
            {form.explanationImage&&<button onClick={()=>setForm(f=>({...f,explanationImage:null}))} style={{fontSize:12,padding:"4px 12px",borderRadius:6,cursor:"pointer",background:"transparent",color:"#C0392B",border:"1px solid #D85A30"}}>Eliminar</button>}
          </div>
          <input ref={explImgRef} type="file" accept="image/*" onChange={handleExplImg} style={{display:"none"}}/>
        </div>
        <div>
          <div style={{fontSize:13,color:"#666",marginBottom:6}}>Imagen de la pregunta (opcional)</div>
          {form.image&&<img src={form.image} alt="" style={{width:"100%",maxHeight:200,objectFit:"cover",borderRadius:6,marginBottom:8,border:"1px solid #ddd"}}/>}
          <input ref={fileRef} type="file" accept="image/*" onChange={handleImg} style={{display:"none"}}/>
          <div style={{display:"flex",gap:8}}>
            <button onClick={()=>fileRef.current.click()} style={{fontSize:13,padding:"6px 14px",borderRadius:6,cursor:"pointer",background:"transparent",border:"1px solid #ddd"}}>Subir imagen</button>
            {form.image&&<button onClick={()=>setForm(f=>({...f,image:null}))} style={{fontSize:13,padding:"6px 14px",borderRadius:6,cursor:"pointer",background:"transparent",color:"#C0392B",border:"1px solid #D85A30"}}>Eliminar</button>}
          </div>
        </div>
        <div style={{display:"flex",gap:8,paddingTop:8}}>
          <button onClick={saveQ} style={{padding:"7px 20px",borderRadius:6,fontSize:13,fontWeight:500,cursor:"pointer",background:"#1D9E75",color:"#fff",border:"none"}}>Guardar</button>
          <button onClick={()=>setView("list")} style={{padding:"7px 16px",borderRadius:6,fontSize:13,cursor:"pointer",background:"transparent",border:"1px solid #ddd"}}>Cancelar</button>
        </div>
      </div>
    </div>
  );
    ...
    <div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1rem",flexWrap:"wrap",gap:8}}>
        <div style={{fontSize:13,color:"#666"}}>
          {db.length} preguntas · <span style={{color:"#0F6E56",fontWeight:600}}>{supervised} supervisadas</span> · <span style={{color:"#BA7517",fontWeight:600}}>{db.length-supervised} pendientes</span>
        </div>
        <button onClick={openNew} style={{padding:"6px 14px",borderRadius:6,fontSize:13,fontWeight:500,cursor:"pointer",background:"#1D9E75",color:"#fff",border:"none"}}>+ Nueva</button>
      </div>
      <div style={{background:"#f9f9f9",borderRadius:6,padding:"12px 14px",marginBottom:"1rem",display:"flex",flexDirection:"column",gap:10,border:"1px solid #eee"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          <span style={{fontSize:12,fontWeight:500,color:"#666",minWidth:80}}>Supervisión:</span>
          {[["todas","Todas"],["supervisadas","Supervisadas"],["pendientes","Pendientes"]].map(([v,l])=>(
            <button key={v} onClick={()=>setFilterSupervised(v)}
              style={{fontSize:12,padding:"3px 12px",borderRadius:20,cursor:"pointer",fontWeight:filterSupervised===v?600:400,
                background:filterSupervised===v?(v==="supervisadas"?"#E1F5EE":v==="pendientes"?"#FEF3DC":"#f0f0f0"):"transparent",
                color:filterSupervised===v?(v==="supervisadas"?"#0F6E56":v==="pendientes"?"#7A4A00":"#333"):"#666",
                border:"1px solid "+(filterSupervised===v?(v==="supervisadas"?"#1D9E75":v==="pendientes"?"#E6A020":"#ccc"):"#ddd")}}>
              {l}
            </button>
          ))}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          <span style={{fontSize:12,fontWeight:500,color:"#666",minWidth:80}}>Tema:</span>
          <select value={filterTopic} onChange={e=>setFilterTopic(e.target.value)} style={{fontSize:12,padding:"4px 8px",borderRadius:6,border:"1px solid #ddd"}}>
        ...
        {(filterSupervised!=="todas"||filterTopic!=="todos")&&(
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:12,color:"#666"}}>{filteredDb.length} resultado{filteredDb.length!==1?"s":""}</span>
            <button onClick={()=>{setFilterSupervised("todas");setFilterTopic("todos");}} style={{fontSize:11,padding:"2px 8px",borderRadius:10,cursor:"pointer",background:"transparent",color:"#666",border:"1px solid #ddd"}}>Limpiar filtros</button>
          </div>
        )}
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {filteredDb.length===0&&<p style={{fontSize:13,color:"#666",padding:"1rem",textAlign:"center"}}>No hay preguntas con estos filtros.</p>}
        {filteredDb.map(q=>{
          const ds=diffStyle(q.difficulty);
          return (
            <div key={q.id} style={{display:"flex",alignItems:"flex-start",gap:12,padding:"10px 14px",borderRadius:6,
              border:"1px solid "+(q.supervised?"#1D9E75":"#ddd"),background:q.supervised?"#F7FDFB":"#fff"}}>
              {q.image&&<img src={q.image} alt="" style={{width:50,height:40,objectFit:"cover",borderRadius:4,flexShrink:0}}/>}
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:13,marginBottom:6,lineHeight:1.4}}>{q.question}</div>
                <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
                  ...
                  {q.topic&&<span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:"#f0f0f0",color:"#666",border:"1px solid #ddd"}}>{q.topic}</span>}
                  <SupervisionTag supervised={q.supervised}/>
                  {isSupervisor&&(
                    <button onClick={()=>toggleSupervised(q.id)}
                      style={{fontSize:11,padding:"2px 10px",borderRadius:10,cursor:"pointer",fontWeight:600,
                        ...
                        border:"1px solid "+(q.supervised?"#D85A30":"#1D9E75")}}>
              ...
              <div style={{display:"flex",gap:6,flexShrink:0}}>
                <button onClick={()=>openEdit(q)} style={{fontSize:12,padding:"4px 10px",borderRadius:6,cursor:"pointer",background:"transparent",color:"#666",border:"1px solid #ddd"}}>Editar</button>
                {confirmDelete===q.id
                  ? <div style={{display:"flex",gap:4,alignItems:"center"}}>
                      <span style={{fontSize:11,color:"#C0392B"}}>¿Seguro?</span>
                      <button onClick={()=>del(q.id)} style={{fontSize:12,padding:"4px 10px",borderRadius:6,cursor:"pointer",background:"#C0392B",color:"#fff",border:"none"}}>Sí</button>
                      <button onClick={()=>setConfirmDelete(null)} style={{fontSize:12,padding:"4px 10px",borderRadius:6,cursor:"pointer",background:"transparent",border:"1px solid #ddd"}}>No</button>
                    </div>
                  : <button onClick={()=>setConfirmDelete(q.id)} style={{fontSize:12,padding:"4px 10px",borderRadius:6,cursor:"pointer",background:"transparent",color:"#C0392B",border:"1px solid #D85A30"}}>Eliminar</button>
                }
              </div>
            </div>
          );
...
// ─── IMPORT VIEW ─────────────────────────────────────────────────────────────
function ImportView({ db, updateDb }) {
  ...
  const handlePdfFile = e => { const f=e.target.files[0]; if(!f) return; setPdfFile(f); setPdfName(f.name); setPdfStatus(""); setPreview(null); };
  ...
  const generateFromDoc = async () => {
    ...
    try {
      const b64 = await new Promise((res,rej)=>{ const r=new FileReader(); r.onload=()=>res(r.result.split(",")[1]); r.onerror=()=>rej(new Error("Error al leer")); r.readAsDataURL(pdfFile); });
      ...
      const prompt = "Eres experto en histología médica. Lee el documento adjunto y genera exactamente "+pdfCount+" preguntas de opción múltiple de nivel \""+pdfDiff+"\" sobre el tema \""+pdfTopic+"\".\n"+
        "Responde SOLO con un array JSON sin texto extra ni backticks:\n"+
        "[{\"question\":\"...\",\"options\":[\"...\",\"...\",\"...\",\"...\"],\"answer\":0,\"explanation\":\"...\"}]\n"+
        "\"answer\" es el índice 0-3. Explicaciones breves (máx 2 frases).";
      const msgContent=[
        {type:"document",source:{type:"base64",media_type:isPdf?"application/pdf":"application/vnd.openxmlformats-officedocument.wordprocessingml.document",data:b64}},
        {type:"text",text:prompt}
      ];
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:2000,messages:[{role:"user",content:msgContent}]})});
      if (!res.ok) { const err=await res.text(); throw new Error("HTTP "+res.status+": "+err.slice(0,200)); }
      const data=await res.json();
      if (!data.content||!Array.isArray(data.content)) throw new Error("Respuesta inesperada.");
      const text=data.content.map(c=>c.text||"").join("");
  ...
  const addPreview = () => { updateDb([...db,...preview]); setPreview(null); setPdfStatus("Se añadieron "+preview.length+" preguntas al banco."); setPdfFile(null); setPdfName(""); };
  ...
  const handleExcel = async e => {
    ...
    try {
      if (isCsv) {
        ...
        setXlStatus("Para Excel instala la librería SheetJS. Por ahora usa CSV.");
      }
    ...
    <div>
      <div style={{display:"flex",gap:8,marginBottom:"1.5rem"}}>
        {[["PDF / Word","pdf"],["Excel / CSV","excel"]].map(([l,v])=>(
          <button key={v} onClick={()=>setTab(v)}
            style={{padding:"6px 16px",borderRadius:6,fontSize:13,cursor:"pointer",
              background:tab===v?"#EBF4FF":"transparent",color:tab===v?"#1a56db":"#666",
              border:tab===v?"1px solid #93c5fd":"1px solid #ddd"}}>
            {l}
          </button>
        ))}
      ...
      {tab==="pdf"&&(
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{background:"#f9f9f9",borderRadius:6,padding:"14px 16px",fontSize:13,color:"#666",lineHeight:1.7,border:"1px solid #eee"}}>
            Sube un PDF o Word con contenido de histología. La IA leerá el documento y generará preguntas basándose en él.
          </div>
          <div onClick={()=>pdfRef.current.click()} style={{border:"1.5px dashed #ddd",borderRadius:6,padding:"24px",textAlign:"center",cursor:"pointer",background:"#fff"}}>
            <div style={{fontSize:28,marginBottom:6}}>📄</div>
            <div style={{fontSize:13,fontWeight:500,marginBottom:4}}>{pdfName||"Haz clic para seleccionar un archivo"}</div>
            <div style={{fontSize:11,color:"#666"}}>PDF o Word (.docx)</div>
            <input ref={pdfRef} type="file" accept=".pdf,.doc,.docx" onChange={handlePdfFile} style={{display:"none"}}/>
          </div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
            <select value={pdfDiff} onChange={e=>setPdfDiff(e.target.value)} style={{fontSize:13,padding:"6px 8px",borderRadius:6,border:"1px solid #ddd"}}>
              {DIFFS.map(d=><option key={d} value={d}>{d}</option>)}
            </select>
            <select value={pdfTopic} onChange={e=>setPdfTopic(e.target.value)} style={{flex:1,minWidth:160,fontSize:13,padding:"6px 8px",borderRadius:6,border:"1px solid #ddd"}}>
              {TOPICS.map(t=><option key={t} value={t}>{t}</option>)}
            </select>
            <select value={pdfCount} onChange={e=>setPdfCount(Number(e.target.value))} style={{fontSize:13,padding:"6px 8px",borderRadius:6,border:"1px solid #ddd"}}>
              {[3,5,8,10].map(n=><option key={n} value={n}>{n} preguntas</option>)}
            </select>
            <button onClick={generateFromDoc} disabled={!pdfFile||pdfGenerating}
              style={{padding:"7px 18px",borderRadius:6,fontSize:13,fontWeight:500,cursor:(!pdfFile||pdfGenerating)?"not-allowed":"pointer",
                opacity:(!pdfFile||pdfGenerating)?0.5:1,background:"#1a56db",color:"#fff",border:"none"}}>
          ...
          {preview&&(
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                <span style={{fontSize:13,fontWeight:600}}>Previsualización — {preview.length} preguntas</span>
                <div style={{display:"flex",gap:8}}>
                  <button onClick={addPreview} style={{fontSize:13,padding:"6px 16px",borderRadius:6,cursor:"pointer",fontWeight:500,background:"#1D9E75",color:"#fff",border:"none"}}>Añadir al banco</button>
                  <button onClick={()=>setPreview(null)} style={{fontSize:13,padding:"6px 12px",borderRadius:6,cursor:"pointer",background:"transparent",border:"1px solid #ddd"}}>Descartar</button>
                </div>
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {preview.map((q,i)=>{
                  const ds=diffStyle(q.difficulty);
                  return (
                    <div key={i} style={{padding:"10px 14px",borderRadius:6,border:"1px solid #ddd",background:"#fff"}}>
                      <div style={{fontSize:13,marginBottom:6,lineHeight:1.4}}>{i+1}. {q.question}</div>
                      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:6}}>
                        <span style={{fontSize:11,padding:"2px 8px",borderRadius:4,fontWeight:600,background:ds.bg,color:ds.color}}>{q.difficulty}</span>
                        <span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:"#f0f0f0",color:"#666",border:"1px solid #ddd"}}>{q.topic}</span>
                      </div>
                      <div style={{display:"flex",flexDirection:"column",gap:3}}>
                        {q.options.map((opt,j)=>(
                          <div key={j} style={{fontSize:12,color:j===q.answer?"#0F6E56":"#666",fontWeight:j===q.answer?600:400}}>
                            {["A","B","C","D"][j]}. {opt} {j===q.answer&&"✓"}
                          </div>
                        ))}
                      </div>
                      {q.explanation&&<div style={{fontSize:12,color:"#666",marginTop:6,fontStyle:"italic"}}>{q.explanation}</div>}
                    </div>
                  );
                })}
              </div>
      ...
      {tab==="excel"&&(
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{background:"#f9f9f9",borderRadius:6,padding:"14px 16px",fontSize:13,color:"#666",lineHeight:1.7,border:"1px solid #eee"}}>
            Importa preguntas desde CSV. Columnas necesarias:<br/>
            <code style={{background:"#f0f0f0",padding:"1px 5px",borderRadius:3,fontSize:12}}>pregunta, opcion_a, opcion_b, opcion_c, opcion_d, respuesta, dificultad, tema, explicacion</code><br/>
            El campo <strong>respuesta</strong> acepta A/B/C/D o 0/1/2/3.
          </div>
          <button onClick={()=>{
            ...
            const a=document.createElement("a");a.href="data:text/csv;charset=utf-8,"+encodeURIComponent(csv);a.download="plantilla_preguntas.csv";a.click();
          }} style={{alignSelf:"flex-start",fontSize:12,padding:"5px 14px",borderRadius:6,cursor:"pointer",background:"transparent",border:"1px solid #ddd"}}>
            Descargar plantilla CSV
          </button>
          <div onClick={()=>xlRef.current.click()} style={{border:"1.5px dashed #ddd",borderRadius:6,padding:"24px",textAlign:"center",cursor:"pointer",background:"#fff"}}>
            <div style={{fontSize:28,marginBottom:6}}>📊</div>
            <div style={{fontSize:13,fontWeight:500,marginBottom:4}}>Haz clic para seleccionar un archivo</div>
            <div style={{fontSize:11,color:"#666"}}>CSV (.csv)</div>
...
function SupervisorMode({ users, updateUsers }) {
  ...
  const del = id => { if(id==="u0"){alert("No se puede eliminar el supervisor principal.");return;} updateUsers(users.filter(u=>u.id!==id)); };
    ...
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1.25rem"}}>
        ...
        <button onClick={()=>{setShowForm(s=>!s);setMsg("");}}
          style={{padding:"6px 14px",borderRadius:6,fontSize:13,fontWeight:500,cursor:"pointer",background:"#1D9E75",color:"#fff",border:"none"}}>
          + Nuevo usuario
        </button>
      </div>
      {showForm&&(
        <div style={{background:"#f9f9f9",borderRadius:6,padding:"14px 16px",marginBottom:"1.25rem",border:"1px solid #eee"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              <input value={form.displayName} onChange={e=>setForm(f=>({...f,displayName:e.target.value}))} placeholder="Nombre completo"
                style={{flex:2,minWidth:140,fontSize:13,padding:"7px 10px",borderRadius:6,border:"1px solid #ddd"}}/>
              <input value={form.username} onChange={e=>setForm(f=>({...f,username:e.target.value}))} placeholder="Usuario (login)"
                style={{flex:1,minWidth:100,fontSize:13,padding:"7px 10px",borderRadius:6,border:"1px solid #ddd"}}/>
              <input value={form.password} onChange={e=>setForm(f=>({...f,password:e.target.value}))} placeholder="Contraseña"
                style={{flex:1,minWidth:100,fontSize:13,padding:"7px 10px",borderRadius:6,border:"1px solid #ddd"}}/>
              <select value={form.role} onChange={e=>setForm(f=>({...f,role:e.target.value}))} style={{fontSize:13,padding:"7px 8px",borderRadius:6,border:"1px solid #ddd"}}>
                {ROLES.map(r=><option key={r} value={r}>{r}</option>)}
              </select>
            </div>
            <div style={{display:"flex",gap:8}}>
              <button onClick={add} style={{padding:"6px 18px",borderRadius:6,fontSize:13,fontWeight:500,cursor:"pointer",background:"#1D9E75",color:"#fff",border:"none"}}>Crear</button>
              <button onClick={()=>setShowForm(false)} style={{padding:"6px 14px",borderRadius:6,fontSize:13,cursor:"pointer",background:"transparent",border:"1px solid #ddd"}}>Cancelar</button>
            </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {users.map(u=>(
          <div key={u.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",borderRadius:6,
            border:"1px solid #ddd",background:u.active?"#fff":"#f9f9f9",opacity:u.active?1:0.7}}>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:14,fontWeight:500,marginBottom:3}}>{u.displayName}</div>
            ...
            <div style={{display:"flex",gap:6,flexShrink:0}}>
              <button onClick={()=>toggleActive(u.id)}
                style={{fontSize:12,padding:"4px 10px",borderRadius:6,cursor:"pointer",background:"transparent",
                  color:u.active?"#BA7517":"#1D9E75",border:"1px solid "+(u.active?"#E6A020":"#1D9E75")}}>
                {u.active?"Desactivar":"Activar"}
              </button>
              {u.id!=="u0"&&(
                <button onClick={()=>del(u.id)}
                  style={{fontSize:12,padding:"4px 10px",borderRadius:6,cursor:"pointer",background:"transparent",color:"#C0392B",border:"1px solid #D85A30"}}>
                  Eliminar
                </button>
              )}
            </div>
